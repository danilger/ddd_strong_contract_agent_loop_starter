...
`.agents/skills/domain-drawio-map/`  
(including `SKILL.md`, `reference.md`, **`example.domain-map.dio`**, `generate-domain-dio.mjs`, `drawio-xml.mjs`, `check-dio-duplicates.mjs`).
...
| Path | Context | Notes |
|------|---------|--------|
| `.agents/examples/server/src/user/docs/.dio` | user (example) | Fuller example (4 use cases, 2 VOs, 2 adapters) |
| `server/src/<slug>/docs/.dio` | live BC | Generated by `npm run docs:domain-map -- <slug>` from `server/` |
...
```bash
npm run docs:domain-map -- auth
npm run docs:domain-map -- account
node ../.agents/skills/domain-drawio-map/check-dio-duplicates.mjs
```

Scripts: `generate-domain-dio.mjs`, `drawio-xml.mjs`, `check-dio-duplicates.mjs` (same folder as this reference).
